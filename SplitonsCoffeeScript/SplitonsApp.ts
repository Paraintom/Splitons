    ...
    function calculateBalances() {
        ...
        p.transactions.forEach(t=>{
            ...
            t.to.forEach(debitor=> {
                result[debitor].balance -= (t.amount / numberOfDebiter);
            });
        });
        return result;
    }
...
function getFakeProject() {
    ...
    p.transactions.push(new Transaction("jean", ["emeline", "antoine"], "Beer", 21));
    p.transactions.push(new Transaction("emeline", ["jean", "antoine", "roger"], "Cab", 32));
    p.transactions.push(new Transaction("emeline", ["roger"], "chewing gum", 32));
