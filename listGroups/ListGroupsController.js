...
angular.module('splitonsApp').controller('ListGroupsController', ['$scope', 'projectsFactory', 'synchFactory', '$location', '$route', '$window', function ($scope, projectsFactory, synchFactory, $location, $route, $window) {
    ...
    $scope.sendViaFastFlicker = function (projectName, projectId) {
        ...
        bootbox.dialog({
            message: "Share the following code with your friends : <b>" + passphrase + "</b><br>" + "When he is listening, click on Button <b>Share</b>." + "<script>addWaveEffect()</script>",
            title: 'Share a Group',
            buttons: {
                cancel: {
                    label: "Exit",
    ...
    $scope.receiveProject = function () {
        bootbox.dialog({
            message: '<div id="joinGroupCode">' + '<input class="bootbox-input bootbox-input-text form-control" autocomplete="off" type="text" placeholder="Enter group code...">' + '</div>',
            title: 'Join a Group',
            buttons: {
                cancel: {
                    label: "Cancel",
...
//# sourceMappingURL=ListGroupsController.js.map